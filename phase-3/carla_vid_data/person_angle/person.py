...
def main():
    ...
    try:
        ...
        camera_transform = carla.Transform(
            carla.Location(
                x=spawn_point.location.x - 2.0,  # 15 meters behind (X axis)
                y=spawn_point.location.y - 2.0,  # 15 meters to the left (Y axis)
                z=spawn_point.location.z + 25.0   # 20 meters up in the air (Z axis)
            ),
            carla.Rotation(
                pitch=-70.0, # Tilt down 45 degrees
                yaw=50.0,    # Turn right 45 degrees to face the spawn point
                roll=0.0
            ) 
        )
