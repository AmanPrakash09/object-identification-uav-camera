...
CAMERA_RADIUS = 1.0                        # 5-meter radius circle
ANGULAR_VELOCITY = CAMERA_SPEED_MPS / CAMERA_RADIUS # Radians per second
...
def main():
    ...
    try:
        ...
        # Spawn Recording Camera (Offset 20 meters to the right)
        camera_bp = blueprint_library.find('sensor.camera.rgb')
        camera_bp.set_attribute('image_size_x', str(IMAGE_WIDTH))
        camera_bp.set_attribute('image_size_y', str(IMAGE_HEIGHT))
        camera_bp.set_attribute('fov', str(FOV))

        initial_cam_x = spawn_point.location.x
        initial_cam_y = spawn_point.location.y + 20.0
        initial_cam_z = spawn_point.location.z + 25.0

        camera_transform = carla.Transform(
            carla.Location(x=initial_cam_x, y=initial_cam_y, z=initial_cam_z),
        ...
        print(f"The drone will orbit in a {CAMERA_RADIUS}m radius circle at {CAMERA_SPEED_KMH} km/h.")
        ...
        while True:
            ...
            time_elapsed = frame_num * DELTA_TIME
            current_cam_transform = camera.get_transform()
            
            # Calculate new X and Y based on the circular path
            # x(t) = initial_x - radius * sin(wt)
            # y(t) = (initial_y + radius) - radius * cos(wt)  <-- Keeps the start perfectly smooth
            current_cam_transform.location.x = initial_cam_x + (CAMERA_RADIUS * math.sin(ANGULAR_VELOCITY * time_elapsed))
            current_cam_transform.location.y = (initial_cam_y - CAMERA_RADIUS) + (CAMERA_RADIUS * math.cos(ANGULAR_VELOCITY * time_elapsed))
