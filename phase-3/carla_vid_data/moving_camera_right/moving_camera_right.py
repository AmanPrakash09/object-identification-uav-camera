...
def get_bbox_and_draw(image, vehicle, camera, K):
    ...
    if len(points_2d) > 0:
        ...
        if width > 0 and height > 0:
            ...
            cv2.putText(image, 'Vehicle', (x_min, max(15, y_min - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
def main():
    ...
    try:
        ...
        # Spawn Vehicle (Swap this back to carlacola if you want!)
        ...
        while running:
            ...
            # Get current camera position, add to its Y value (Right), and set it back
            ...
            # Bounding boxes calculate dynamically based on the camera's new position every frame!
