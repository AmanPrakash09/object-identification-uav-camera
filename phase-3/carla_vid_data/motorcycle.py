...
def run_simulation():
    ...
    try:
        ...
        # Safely turn off synchronous modes
        try:
            if traffic_manager:
        ...
        # Safely destroy actors only if they are still alive
...
def main():
    while True:
        try:
            run_simulation()
            # If run_simulation finishes naturally (user pressed ESC or closed window)
            print("\nExiting script.")
            break
            
        except KeyboardInterrupt:
            # User pressed Ctrl+C in the terminal
            print("\nUser forcefully stopped the script via terminal. Exiting.")
            break
            
        except Exception as e:
            # Something crashed (CARLA disconnected, actor vanished, etc.)
            print(f"\n[CRASH DETECTED] Error encountered: {e}")
