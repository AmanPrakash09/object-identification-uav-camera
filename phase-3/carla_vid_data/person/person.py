...
def get_bbox_and_draw(image, actor, camera, K):
    ...
    if len(points_2d) > 0:
        ...
        if width > 0 and height > 0:
            ...
            cv2.putText(image, 'Pedestrian', (x_min, max(15, y_min - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
def main():
    ...
    try:
        ...
        # --- SPAWN PEDESTRIAN ---
        walker_bp = blueprint_library.filter('walker.pedestrian.*')[0] # Grabs the first pedestrian model
        spawn_point = world.get_map().get_spawn_points()[0]
        spawn_point.location.z += 1.0 # Drop them slightly so they don't clip into the floor
        
        walker = world.spawn_actor(walker_bp, spawn_point)
        actor_list.append(walker)

        bbox = walker.bounding_box
        walker_length = bbox.extent.x * 2.0
        walker_width = bbox.extent.y * 2.0
        walker_height = bbox.extent.z * 2.0
        
        print(f"\n--- ACTOR SPECS ---")
        print(f"Actor: Pedestrian")
        print(f"Length: {walker_length:.2f} meters")
        print(f"Width:  {walker_width:.2f} meters")
        print(f"Height: {walker_height:.2f} meters")
        print(f"---------------------\n")
        
        spectator = world.get_spectator()

        # Spawn Recording Camera (Kept at 25m)
        ...
        print("\nReady! CLICK THE SMALL PYGAME WINDOW to walk using WASD. Hold SPACE to run. Press ESC to stop.\n")
        
        while running:
            world.tick()
            
            # --- PYGAME KEYBOARD CONTROL FOR WALKER ---
            ...
            # Walkers use a direction vector and a speed
            control = carla.WalkerControl()
            direction = carla.Vector3D(0, 0, 0)
            speed = 0.0
            
            if keys[K_w]: direction.x += 1.0
            if keys[K_s]: direction.x -= 1.0
            if keys[K_a]: direction.y -= 1.0
            if keys[K_d]: direction.y += 1.0
                
            # Normalize the vector so diagonal walking isn't faster
            length = math.sqrt(direction.x**2 + direction.y**2)
            if length > 0.0:
                direction.x /= length
                direction.y /= length
                speed = 2.0 # Normal walking speed (m/s)
            ...
            # --- SPECTATOR FOLLOWS WALKER ---
            walker_transform = walker.get_transform()
            # Moved spectator right behind the shoulder of the pedestrian
            spectator_offset = -3.0 * walker_transform.rotation.get_forward_vector() + carla.Location(z=2.0)
            spectator_transform = carla.Transform(walker_transform.location + spectator_offset, walker_transform.rotation)
            ...
            img_with_box, bbox = get_bbox_and_draw(img_draw, walker, camera, K)
            ...
            display.blit(font.render("🚨 CLICK THIS WINDOW TO WALK 🚨", True, (255, 80, 80)), (20, 20))
            display.blit(font.render("WASD: Move | Hold SPACE: Run", True, (200, 200, 200)), (20, 70))
